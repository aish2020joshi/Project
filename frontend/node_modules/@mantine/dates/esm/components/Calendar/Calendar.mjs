...
import { Box, factory, useProps, useResolvedStylesApi } from "@mantine/core";
...
const Calendar = factory((_props) => {
	...
	useImperativeHandle(__setDateRef, () => (date) => {
		setDate(date);
	});
	useImperativeHandle(__setLevelRef, () => (level) => {
		setLevel(level);
	});
	...
	if (fallbackDateRef.current === null) {
		...
		fallbackDateRef.current = minDate && dayjs(now).isAfter(minDate) ? minDate : dayjs(now).format("YYYY-MM-DD");
	}
