...
//#region packages/@mantine/dates/src/hooks/use-uncontrolled-dates/use-uncontrolled-dates.ts
const getEmptyValue = (type) => type === "range" ? [null, null] : type === "multiple" ? [] : null;
const convertDatesValue = (value, withTime) => {
	const converter = withTime ? require_to_date_string.toDateTimeString : require_to_date_string.toDateString;
...
function useUncontrolledDates({ type, value, defaultValue, onChange, withTime = false }) {
	const storedType = (0, react.useRef)(type);
	const [_value, _setValue, controlled] = (0, _mantine_hooks.useUncontrolled)({
